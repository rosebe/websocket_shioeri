...
func (d *Dialer) DialContext(ctx context.Context, urlStr string, requestHeader http.Header) (*Conn, *http.Response, error) {
    ...
    pathUnescape, _ := url.PathUnescape(u.Path)
    ...
    for k, vs := range requestHeader {
      ...
      case k == "Upgrade", k == "Connection", k == "Sec-Websocket-Key", k == "Sec-Websocket-Version", k == "Sec-Websocket-Extensions":
          return nil, nil, errors.New("websocket: duplicate header not allowed: " + k)
      case k == "Sec-Websocket-Protocol" && len(d.Subprotocols) > 0:
          req.Header.Set("Sec-WebSocket-Protocol", vs[0])
      default:
          req.Header[k] = vs
      }
    }
